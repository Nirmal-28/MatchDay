...
const RANK_ACCENT = ["text-accent-yellow", "text-stone-300", "text-accent-orange"];
        ...
        <Reveal>
          <div className="overflow-x-auto rounded-md border border-stone-200">
            <table className="w-full text-left text-sm">
              ...
              <StaggerList as="tbody" className="divide-y divide-stone-100">
                {rows.map((p, i) => (
                  <StaggerItem key={p.id} as="tr">
                    ...
                    <td className="px-3 py-2.5">
                      <Link to={`/p/${p.id}`} className="font-medium text-stone-800 hover:text-teal-700 hover:underline">
                        {p.name}
                      </Link>
                      {p.city && (
                        <span className="ml-2 inline-flex items-center gap-0.5 text-xs text-stone-400">
                          <MapPin size={10} />{p.city}
                        </span>
                      )}
                    </td>
                    <td className="px-3 py-2.5 text-center font-mono text-stone-600">{p.played}</td>
                    <td className="px-3 py-2.5 text-center font-mono font-semibold text-stone-800">{p.won}</td>
                    <td className={cx("px-3 py-2.5 text-center font-mono font-semibold", i < 3 ? "text-accent-teal" : "text-stone-600")}>
                      {p.winPct}%
                    </td>
                  </StaggerItem>
                ))}
