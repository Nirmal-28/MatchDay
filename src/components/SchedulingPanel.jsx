    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-xs font-semibold uppercase tracking-wide text-ink-2">Schedule quality</div>
            <div className="flex items-baseline gap-2">
              <span className="font-display text-3xl font-bold text-ink">{quality.score}</span>
              <span className="text-sm text-ink-3">/ 100</span>
              <Badge tone={board.tournament?.schedule_published ? "emerald" : "amber"}>{board.tournament?.schedule_published ? "Published" : "Draft"}</Badge>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Btn size="sm" variant="secondary" icon={Settings2} onClick={() => setSettingsOpen((s) => !s)}>Configure</Btn>
            <Btn size="sm" icon={Wand2} disabled={busy} onClick={doOptimize}>Optimize Schedule</Btn>
            <Btn size="sm" variant="primary" icon={Rocket} disabled={busy || scheduledMatches.length === 0} onClick={doPublish}>Publish Schedule</Btn>
          </div>
        </div>

        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[
            { label: "Matches scheduled", value: quality.scheduledCount },
            { label: "Avg court utilization", value: `${quality.avgUtilization}%` },
            { label: "Hard conflicts", value: quality.hardConflicts },
            { label: "Avg player rest", value: quality.avgRestMins != null ? `${quality.avgRestMins} min` : "—" },
          ].map((s) => (
            <div key={s.label} className="rounded-md border border-line bg-surface-2 p-2 text-center">
        ...
        <div className="mt-3 space-y-1">
          {quality.checklist.map((item, i) => (
            <div key={i} className={`flex items-center gap-1.5 text-xs ${item.ok ? "text-emerald-400" : "text-amber-400"}`}>
              {item.ok ? <CheckCircle2 size={13} /> : <AlertTriangle size={13} />} {item.label}
            </div>
          ))}
        </div>
      </Card>
      ...
      {settingsOpen && (
        <Card className="p-4">
          <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-ink-2">Scheduling constraints</div>
          ...
          <div className="mt-4 mb-2 text-xs font-semibold uppercase tracking-wide text-ink-2">Court availability</div>
          ...
          {dates.length > 1 && (
            <>
              <div className="mt-4 mb-2 text-xs font-semibold uppercase tracking-wide text-ink-2">Per-day court overrides (multi-day)</div>
      ...
      {activeCourts.length > 0 && (
        <Card className="p-4">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-2">Court utilization</div>
      ...
      {(view === "timeline" || view === "list") && (() => {
        const ordered = [...scheduledMatches].sort((a, b) => view === "list"
                  ...
                  <div className={`rounded-lg border bg-surface p-3 sm:hidden ${
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 truncate text-[11px] uppercase tracking-wide text-ink-3">
      ...
      {(quality.conflicts || []).length > 0 && (
        <Card className="p-4">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-2">Conflicts &amp; suggestions</div>
          <div className="space-y-1.5">
