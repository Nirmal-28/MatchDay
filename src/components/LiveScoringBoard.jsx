...
export default function LiveScoringBoard({ matches, events, entriesById, onStart, onScore, onUndo, onRetire }) {
  ...
  if (openMatch) {
    return (
      <div>
        <button className="mb-3 flex items-center gap-1 text-xs font-medium text-ink-2 hover:text-ink" onClick={() => setOpenMatchId(null)}><ChevronLeft size={14} /> All courts</button>
        {justFinished ? (
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {active.map((m) => {
        ...
        return (
          <button key={m.id} onClick={() => setOpenMatchId(m.id)} className="rounded-lg border border-line bg-surface p-3.5 text-left shadow-sm hover:border-accent-teal/50 hover:shadow-md transition-all">
