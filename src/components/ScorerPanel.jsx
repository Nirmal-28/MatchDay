      ...
      {!isLive ? (
        ...
        <div className="p-4">
            ...
            {[["A", a], ["B", b]].map(([side, e]) => {
              ...
              return (
                <div key={side} className="flex flex-col items-center gap-2 rounded-md border border-white/10 bg-white/5 py-4">
                  <div className="px-2 text-center text-sm font-medium text-stone-200">{entryName(e)}</div>
                  <motion.div
                    key={value}
                  ...
                  <div className="flex gap-2">
                    <button className="flex h-9 w-9 items-center justify-center rounded-md border border-white/15 text-stone-300 hover:bg-white/10 disabled:opacity-30" onClick={() => onScore(match.id, side, -1)}><Minus size={16} /></button>
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Btn size="sm" variant="ghost" icon={RotateCcw} className="text-stone-300 hover:bg-white/10 hover:text-white" onClick={() => onUndo(match.id)}>Undo game</Btn>
              {confirmRetire ? (
                <div className="flex items-center gap-1 text-xs">
                  <span className="text-stone-300">{entryShort(confirmRetire === "A" ? a : b)} retires?</span>
                  <Btn size="sm" variant="danger" onClick={() => { onRetire(match.id, confirmRetire); setConfirmRetire(null); }}>Confirm</Btn>
                  <Btn size="sm" variant="ghost" className="text-stone-300 hover:bg-white/10 hover:text-white" onClick={() => setConfirmRetire(null)}>Cancel</Btn>
                </div>
              ) : (
                <div className="flex gap-1">
                  <Btn size="sm" variant="ghost" className="text-stone-300 hover:bg-white/10 hover:text-white" onClick={() => setConfirmRetire("A")}>A retires</Btn>
                  <Btn size="sm" variant="ghost" className="text-stone-300 hover:bg-white/10 hover:text-white" onClick={() => setConfirmRetire("B")}>B retires</Btn>
                </div>
              )}
            </div>
          </div>
